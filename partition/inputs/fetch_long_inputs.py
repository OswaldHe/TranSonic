...
def main(argv: list[str] | None = None) -> int:
    ...
    args.out.write_text(f"{body}\n")

    # JSONL stays pure JSON, so provenance goes in a sidecar rather than a comment
    # line the readers would all have to know to skip.
    digest = hashlib.sha256(body.encode()).hexdigest()[:16]
    args.out.with_suffix(".provenance.json").write_text(json.dumps({
        "generator": "fetch_long_inputs.py",
        "source": args.source,
        "seed": args.seed,
