...
def series(task):
    ...
    best_by_iter = task.get("best_by_iter", {})
    values = [1.0]
    last = 1.0
    for iteration in range(1, MAX_ITER + 1):
        speedup = best_by_iter.get(str(iteration))
...
all_series = [series(task) for task in tasks.values()]
...
print(
    f"\nfinal: median {median[-1]:.2f}x "
    f"geomean {geometric_mean[-1]:.2f}x over {len(tasks)} tasks"
)
...
ax.set_title(
    "Median speedup (Claude Opus 4.8)",
    fontsize=11,
    fontweight="bold",
    pad=10,
)
...
output = os.path.abspath(
    os.path.join(HERE, "..", "..", "assets", "progression_algotune.png")
)
